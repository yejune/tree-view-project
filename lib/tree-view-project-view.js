...
export default class TreeViewProjectView {
  ...
  initialize() {
    //this.treeView.find('.tree-view-scroller').css('background', this.treeView.find('.tree-view').css('background'));
    //this.treeView.prepend(_this.treeView);

    //this.treeView = document.getElementsByClassName('tree-view-resizer tool-panel')[0];
    //
    //

    view = $(this.treeView.element);
    //treeView = document.getElementsByClassName('tree-view-resizer tool-panel')[0];
    if (view.find('.tree-view-project')) {
      //treeView.removeChild(treeView.getElementsByClassName('tree-view-project')[0]);
      view.find('.tree-view-project').remove();
    }

    this.message = this.render(this.getProjects());
    this.message.classList.add('tree-view-project');

    view.prepend($(this.message));

    view.find('.tree-view-project > .list-tree.has-collapsable-children .list-nested-item > .list-item').on("click", function() {
  ...
  isVisible() {
    ...
    return this.message.classList.contains('show');
  }
...
function onSelect(project) {
  console.log('log treeview select');
  console.log(atom.workspace.project);
  // remove all current directories that do not belong to the
  // selected project.
  atom.workspace.project.getDirectories().filter(function(directory) {
    return project.paths.indexOf(directory.path) === -1;
  }).forEach(function(directory) {
    atom.workspace.project.removePath(directory.path);
    console.log('project remove '+directory.path)
  });

  // next add all remaining project directories
  project.paths.forEach(function(eachPath) {
    if (!atom.workspace.project.contains(eachPath)) {
      atom.workspace.project.setPaths([eachPath]);
      console.log('project add '+eachPath)
    }
  });
}
